...
//根据不同的类型，建立不同的输入框
function furnInp(param,key,value=null){
    //前端数据和图形数据相互补全
...
function inputFragment(label,valType,key,value){
    return `
            <div class="liv-group">
...
function selectionFragment(label,valType,key,value,text){
    return `
            <div class="liv-group">
                <label class="liv-lab">${label}</label>
...
//解析输入框数据
function parseFurnParam(valType,val,numFn='parseUnit'){
    switch (valType){
        case 'number':
